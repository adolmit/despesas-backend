...
public interface MovimentacaoRepository extends JpaRepository<MovimentacaoEntity, Long> {
    @Query("select  m.mes AS reporteMes, sum(m.valor) AS reporteValor from MovimentacaoEntity AS m " +
            " group by m.mes order by m.mes ASC")
    List<ReporteMesDTO> getReportByMonth();

    @Query("select  C.nome AS reporteCategoria, sum(m.valor) AS reporteValor from MovimentacaoEntity AS m " +
